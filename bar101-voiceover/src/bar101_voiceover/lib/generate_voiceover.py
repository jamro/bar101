...
def generate_voiceover(group, character, text, model=None):
  # create md5 hash
  ...
  target_dir = os.path.join(os.path.dirname(__file__), "..", "..", "..", "voiceovers", "storytree", group)
  ...
  if os.path.exists(target_file_mp3):
    print(f"Voiceover already exists: {target_file_mp3}. Skipping generation...")
    return
